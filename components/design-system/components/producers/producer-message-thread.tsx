...
type MessageItem = {
  id: string;
  body: string;
  senderRole: "student" | "producer";
  createdAt: string;
};

const urlLike = /(https?:\/\/|www\.)/i;

export function ProducerMessageThread({
  producerSlug,
  producerName,
}: {
  producerSlug: string;
  ...
  const [producerUserId, setProducerUserId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`/api/producer-messages/thread?producerSlug=${encodeURIComponent(producerSlug)}`)
      .then((res) => res.json())
      .then((json) => {
        if (!json.ok) {
        ...
        setProducerUserId(json.producer?.userId || null);
  ...
  const hasProducerReply = messages.some((msg) => msg.senderRole === "producer");
  const studentName =
    (session?.user?.name as string | undefined) ||
    (session?.user?.email as string | undefined) ||
    "Öğrenci";
  const whatsappLink = useMemo(() => {
    if (!whatsapp?.enabled || !whatsapp.number || !hasProducerReply) return null;
  ...
  }, [whatsapp, hasProducerReply, studentName]);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("/api/producer-messages/send", {
        ...
        body: JSON.stringify({ producerSlug, body }),
      });
  ...
  const handleRequest = async () => {
    if (!canRequest || requestSending || !producerUserId) return;
    ...
    try {
      const res = await fetch("/api/producer-message", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ producerUserId, message: requestBody }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(json.error || "Mesaj gönderilemedi.");
      ...
      setRequestBody("");
      setShowModal(false);
      setInfo("Mesajın iletildi. Üretici onaylarsa konuşma başlar.");
    } catch {
      setError("Mesaj gönderilemedi.");
    } finally {
      setRequestSending(false);
    ...
    return (
      <Card id="messages" className="space-y-4 p-5">
        ...
        <p className="text-sm text-[var(--color-muted)]">
          {producerName} ile mesajlaşmak için giriş yapmanız gerekir.
        </p>
        <Button asChild size="sm">
          <Link href={`/login?redirect=/uretim/${producerSlug}#messages`}>Giriş yap</Link>
        </Button>
      </Card>
    );
  }
    ...
    <Card id="messages" className="space-y-4 p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-xs text-[var(--color-muted)]">
            Bu sohbet yalnızca {producerName} ile aranızdadır.
          </p>
        </div>
        {threadId ? <ThreadComplaint threadType="producer" threadId={threadId} /> : null}
      </div>
      ...
      {!threadId && request?.status === "pending" ? (
        <div className="space-y-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-secondary)] p-3 text-sm text-[var(--color-primary)]">
          <p>İlk mesajın iletildi. Üretici onaylarsa konuşma başlar.</p>
          <p className="text-xs text-[var(--color-muted)]">
            ...
            <>
              ...
              {whatsappLink ? (
                <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-secondary)] px-3 py-2 text-xs text-[var(--color-muted)]">
                  <p>Üretici WhatsApp ile devam etmeye izin veriyor.</p>
                  <div className="mt-2">
                    <Button
                      asChild
                      size="sm"
              ...
              <div className="space-y-2">
                <label htmlFor="producer-message-thread" className="text-xs font-semibold text-[var(--color-muted)]">

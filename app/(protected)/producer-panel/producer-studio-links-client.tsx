...
export function ProducerStudioLinksClient({ initialLinks }: Props) {
  ...
  const runSearch = async () => {
    ...
    setSearching(true);
    try {
      const res = await fetch(`/api/producer-panel/studios?query=${encodeURIComponent(trimmed)}`);
  ...
  return (
    <Card className="space-y-5 p-6">
      <div className="space-y-1">
        <h2 className="text-base font-semibold text-[var(--color-primary)]">Çalıştığı stüdyolar</h2>
        <p className="text-sm text-[var(--color-muted)]">
          Üretim yapmak istediğin stüdyoları ekleyebilirsin. Talep gönderdiğinde stüdyo sahibi onaylar veya reddeder.
        </p>
      </div>

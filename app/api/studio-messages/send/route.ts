...
export async function POST(req: Request) {
  ...
  const perHour = rateLimit(`studio-owner-msg:${userId}`, 20, 60 * 60 * 1000);
  if (!perHour.ok) {
  ...
  const thread = await prisma.studioThread.findUnique({
    ...
    include: {
      studio: { select: { ownerEmail: true, name: true } },
  ...
  if (resend && studentEmail) {
    const studentName =
      thread.studentUser.fullName || thread.studentUser.name || thread.studentUser.email || "Kullanıcı";
    const studioName = thread.studio.name || "Stüdyo";
    const threadLink = `${baseUrl}/messages?studioThread=${thread.id}`;
    const text = `${studioName} sana yeni mesaj gönderdi.\n\nMerhaba ${studentName},\n\nMesaj: ${snippet(messageBody)}\n\nKonuşmaya devam etmek için: ${threadLink}\n`;
    try {
      ...
      console.error("studio reply email error", err);
    }
  }

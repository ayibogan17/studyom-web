...
export async function POST(req: Request) {
  ...
  const link = await prisma.producerStudioLink.upsert({
    where: { producerUserId_studioId: { producerUserId: userId, studioId: studio.id } },
    create: { producerUserId: userId, studioId: studio.id, status: "pending" },
    update: { status: "pending" },
    include: { studio: { select: { id: true, name: true, city: true, district: true } } },
  });

  const producerName = dbUser.fullName || dbUser.name || dbUser.email || "Bir üretici";
  const linkUrl = `${process.env.NEXTAUTH_URL || process.env.AUTH_URL || "http://localhost:3000"}/notifications`;
  const emailText = `Merhaba,\n\n${producerName} stüdyonuzda üretim yapmak istediğini bildirdi.\nUygun görürseniz onaylayabilir veya reddedebilirsiniz.\n\nTalebi yönetmek için: ${linkUrl}\n`;
  await notifyUser(studio.ownerEmail, "Yeni üretici stüdyo talebi", emailText);

...
export function TeacherGalleryClient({ initialUrls = [] }: Props) {
  ...
  const saveGallery = async (next: string[]) => {
    const res = await fetch("/api/teacher-panel/photos", {
      method: "POST",
  ...
  return (
    <Card className="space-y-4 p-6">
      ...
      <p className="text-xs text-[var(--color-muted)]">
        Enstrümanınız, stüdyonuz veya öğrencileriniz ile fotoğraflarınız varsa ekleyebilirsiniz.
      </p>
      <p className="text-xs text-[var(--color-muted)]">
        Eklenen fotoğraflar, Hocalar profilinde görünebilir.
      </p>
          ...
          {photos.map((url, idx) => (
            <div key={`${url}-${idx}`} className="space-y-2">
              <div className="aspect-square overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-secondary)]">
                <img src={url} alt="Hoca fotoğrafı" className="h-full w-full object-cover" />
              </div>
              <Button
                size="sm"
                variant="ghost"

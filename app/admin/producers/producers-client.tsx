...
export default function ProducersClient({ items }: { items: ProducerItem[] }) {
  const [rows, setRows] = useState(items);
  const [saving, setSaving] = useState<number | null>(null);
  const [openRow, setOpenRow] = useState<number | null>(null);

  const updateRow = (id: number, patch: Partial<ProducerItem>) => {
  ...
  const publish = (row: ProducerItem) =>
    updateListing(row.id, { visibilityStatus: "published" });
  const hide = (row: ProducerItem) =>
    updateListing(row.id, { visibilityStatus: "hidden" });
  const markNeedsChanges = (row: ProducerItem) =>
  ...
  return (
    <div className="space-y-3">
      {rows.map((row) => (
        <Fragment key={row.id}>
          <div className="rounded-xl border border-[var(--color-border)] p-3">
            ...
            <ProducerDetails data={row.data} userCity={row.user?.city || null} />
          </div>
          {openRow === row.id ? (
      ...
      {rows.length === 0 ? <p className="text-sm text-[var(--color-muted)]">Kayıt yok.</p> : null}
    </div>
  );
}

function ProducerDetails({ data, userCity }: { data: unknown; userCity: string | null }) {
  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="grid gap-2 text-xs text-[var(--color-muted)] sm:grid-cols-2 lg:grid-cols-3">
        <Field label="Üretim alanları" value={<ChipList items={toStringArray(payload.areas)} />} />
        <Field label="Çalışma türü" value={<ChipList items={toStringArray(payload.workTypes)} />} />
        <Field label="Mod" value={<ChipList items={toStringArray(payload.modes)} />} />
        <Field label="Türler" value={<ChipList items={toStringArray(payload.genres)} />} />
        ...
        <Field label="Proje" value={toString(payload.projects) || "-"} />
      </div>
      <div>
